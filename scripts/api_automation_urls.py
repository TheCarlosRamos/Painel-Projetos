...
def read_url_titles(file_path):
    """Lê os títulos das URLs do arquivo de texto."""
    url_titles = {}
    with open(file_path, 'r', encoding='utf-8') as file:
...
def make_api_call(guid, url_code, session=None):
    ...
    try:
        ...
        error_msg = f"Erro ao acessar a API para GUID {guid} e URL {url_code}: {str(e)}"
        print(error_msg)
...
def save_response(guid, url_code, url_title, data, output_dir="responses"):
...
def main():
    ...
    urls_file = "urls.csv"
    url_titles_file = "lista_de_urls.txt"
    ...
    try:
        ...
        print(f"Encontradas {len(urls)} URLs no arquivo {urls_file}")
        if not urls:
            print("Nenhuma URL encontrada no arquivo.")
            return
    except Exception as e:
        print(f"Erro ao ler o arquivo {urls_file}: {e}")
        return

    # Ler os títulos das URLs
    try:
        url_titles = read_url_titles(url_titles_file)
        print(f"Encontrados {len(url_titles)} títulos de URLs no arquivo {url_titles_file}")
    except Exception as e:
        print(f"Erro ao ler o arquivo {url_titles_file}: {e}")
        url_titles = {}

    # Fazer as chamadas à API para cada combinação de GUID e URL
    total_requests = len(guids) * len(urls)
    ...
    try:
        for i, guid in enumerate(guids, 1):
            for j, url_code in enumerate(urls, 1):
                print(f"\n[{'='*50}]")
                print(f"Processando GUID {i}/{len(guids)}, URL {j}/{len(urls)}")
                print(f"GUID: {guid}")
                print(f"URL: {url_code}")
                
                # Obter o título da URL se disponível
                url_title = url_titles.get(url_code, url_code)
                print(f"Título: {url_title}")
