...
def make_api_call(guid, session=None):
    """Faz uma chamada à API para a URL 2000728 (proximas etapas do projeto)."""
    base_url = "https://api.sif-source.org"
    endpoint = f"/projects/{guid}/questions/search/2000728"
    ...
    try:
        ...
        error_file = os.path.join(error_dir, f"error_{guid}_2000728.json")
        ...
        error_msg = f"Erro ao acessar a API para GUID {guid} e URL 2000728: {str(e)}"
        print(error_msg)
...
def save_response(guid, data, output_dir="../responses_2000728"):
    ...
    title_safe = "proximas etapas do projeto".replace(' ', '_').replace('/', '_').replace('(', '').replace(')', '').lower()
    filename = f"{output_dir}/response_{safe_guid}_2000728_{title_safe}.json"
...
def main():
    """Função principal para processar todos os GUIDs com a URL 2000728 (proximas etapas do projeto)."""
    ...
    print(f"\nProcessando URL 2000728 - proximas etapas do projeto")
    print(f"Total de chamadas a serem feitas: {len(guids)}")
    print("Pressione Ctrl+C para interromper a qualquer momento\n")
    
    # Criar arquivo de log
    log_file = os.path.join(error_dir, f"execution_log_2000728.txt")
    start_time = time.strftime("%Y-%m-%d %H:%M:%S")
    
    with open(log_file, 'a', encoding='utf-8') as log:
        log.write(f"\n{'='*50}\n")
        log.write(f"Início da execução - URL 2000728 (proximas etapas do projeto): {start_time}\n")
    ...
    try:
        ...
        print("="*50)
        print(f"URL: 2000728 - proximas etapas do projeto")
