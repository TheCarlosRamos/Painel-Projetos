...
def make_api_call(guid, session=None):
    """Faz uma chamada à API para a URL 2001218 (status dos estudos)."""
    base_url = "https://api.sif-source.org"
    endpoint = f"/projects/{guid}/questions/search/2001218"
    ...
    try:
        ...
        error_file = os.path.join(error_dir, f"error_{guid}_2001218.json")
        ...
        error_msg = f"Erro ao acessar a API para GUID {guid} e URL 2001218: {str(e)}"
        print(error_msg)
...
def save_response(guid, data, output_dir="../responses_2001218"):
    ...
    title_safe = "status dos estudos".replace(' ', '_').replace('/', '_').replace('(', '').replace(')', '').lower()
    filename = f"{output_dir}/response_{safe_guid}_2001218_{title_safe}.json"
...
def main():
    """Função principal para processar todos os GUIDs com a URL 2001218 (status dos estudos)."""
    ...
    print(f"\nProcessando URL 2001218 - status dos estudos")
    print(f"Total de chamadas a serem feitas: {len(guids)}")
    print("Pressione Ctrl+C para interromper a qualquer momento\n")
    
    # Criar arquivo de log
    log_file = os.path.join(error_dir, f"execution_log_2001218.txt")
    start_time = time.strftime("%Y-%m-%d %H:%M:%S")
    
    with open(log_file, 'a', encoding='utf-8') as log:
        log.write(f"\n{'='*50}\n")
        log.write(f"Início da execução - URL 2001218 (status dos estudos): {start_time}\n")
    ...
    try:
        ...
        print("="*50)
        print(f"URL: 2001218 - status dos estudos")
