...
def csv_to_excel_complete():
    """Converte o arquivo CSV completo para Excel."""
    
    # Caminhos dos arquivos
    base_dir = os.path.dirname(os.path.abspath(__file__))
    csv_file = os.path.join(base_dir, '..', 'projetos_completos.csv')
    excel_file = os.path.join(base_dir, '..', 'projetos_completos_final.xlsx')
    ...
    try:
        ...
        with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name='Projetos Completos', index=False)
            
            # Ajustar largura das colunas
            worksheet = writer.sheets['Projetos Completos']
        ...
        print(f"   - Projetos por setor: {df['setor'].value_counts().to_dict()}")
        print(f"   - Projetos por organização: {df['organizacao'].value_counts().to_dict()}")
